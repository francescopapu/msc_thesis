...
def main(csv_file, put, instances, cut_off, step, out_file):
  #Read the results
      ...
      for cov_type in ['b_abs', 'b_per', 'l_abs', 'l_per']:
        ...
        for time in range(1, cut_off + 1, step):
          ...
          for instance in range(0, instances, 1):
            #get instance-specific data frame
            df2 = df1[df1['instance'] == instance]
            ...
            #update total coverage and #instances
            cov_total += df3.tail(1).iloc[0, 5]
            instance_count += 1
          
          #add a new row
          mean_list.append((subject, fuzzer, cov_type, time, cov_total / instance_count))
  ...
  for i, ax in enumerate(fig.axes):
    ax.legend(('AFL', 'AFLGO'), loc='lower right')
    ax.grid()
...
if __name__ == '__main__':
    ...
    main(args.csv_file, args.put, 9, args.cut_off, args.step, args.out_file)
