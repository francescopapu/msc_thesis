...
make CFLAGS="-fprofile-arcs -ftest-coverage -targets=$TMP_DIR/BBtargets.txt -outdir=$TMP_DIR -flto -fuse-ld=gold -Wl,-plugin-opt=save-temps" CC="$AFLGO/afl-clang-fast" CFLAGS+="-g" LDFLAGS+="-g -fprofile-arcs"
...
#$AFLGO/scripts/genDistance.sh $PWD/src $TMP_DIR bzip2
python3 $AFLGO/scripts/gen_distance_fast.py $PWD/src/src $TMP_DIR bzip2
...
make CFLAGS="-fprofile-arcs -ftest-coverage -distance=$TMP_DIR/distance.cfg.txt" CC="$AFLGO/afl-clang-fast" CFLAGS+="-g" LDFLAGS+="-g -fprofile-arcs"
# make install
#
...
# do
#   echo -e  "$entry" | sed 's/.*bzipS\///' >> all_bugs_aflgo.txt
# done
#
