...
make CFLAGS="-targets=$TMP_DIR/BBtargets.txt -outdir=$TMP_DIR -flto -fuse-ld=gold -Wl,-plugin-opt=save-temps" CC="$AFLGO/afl-clang-fast" CFLAGS+="-g -fprofile-arcs -ftest-coverage" LDFLAGS+="-g -fprofile-arcs"
...
#$AFLGO/scripts/genDistance.sh $PWD/src $TMP_DIR pcre2grep
python3 $AFLGO/scripts/gen_distance_fast.py $PWD/src/src $TMP_DIR pcre2grep
...
make CFLAGS="-distance=$TMP_DIR/distance.cfg.txt" CC="$AFLGO/afl-clang-fast" CFLAGS+="-g -fprofile-arcs -ftest-coverage" LDFLAGS+="-g -fprofile-arcs"
# make install
#
...
# ./aflgo_rebuilt/bin/${PROG} ${PROGOPT1} ${INPUT_CLEAN} ${PROGOPT2} &> /dev/null
# rv=$?
# if [ $rv -lt 128 ]; then
...
# do
#   echo -e  "$entry" | sed 's/.*pcreS2\///' >> all_bugs_aflgo.txt
# done
#
# echo "Validating bugs..."
# cat all_bugs_aflgo.txt | while read line ; do
#     INPUT_FUZZ=$(printf "$INPUT_PATTERN" $line)
#     { ./aflgo_rebuilt/bin/${PROG} ${PROGOPT1} ${INPUT_FUZZ} ${PROGOPT2} ; } &> /dev/null
#     echo $line $?
...
# cat all_bugs_aflgo.txt | while read line ; do
#     INPUT_FUZZ=$(printf "$INPUT_PATTERN" $line)
#     { gdb -batch -ex "run ${PROGOPT1} ${INPUT_FUZZ} ${PROGOPT2}" -ex "bt" ./aflgo_rebuilt/bin/${PROG}  2>&1 | grep -v ^"No stack."$ ; } &> backtrace_aflgo/$line.txt
# done
