...
make CFLAGS="-targets=$TMP_DIR/BBtargets.txt -outdir=$TMP_DIR -flto -fuse-ld=gold -Wl,-plugin-opt=save-temps" CC="$AFLGO/afl-clang-fast" CFLAGS+="-g -w" CFLAGS+="-m32" LDFLAGS+="-g"
...
make CFLAGS="-distance=$TMP_DIR/distance.cfg.txt" CC="$AFLGO/afl-clang-fast" CFLAGS+="-g -w" CFLAGS+="-m32" LDFLAGS+="-g" -j 16
make install
